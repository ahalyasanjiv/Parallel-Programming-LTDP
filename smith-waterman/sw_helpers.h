...
char* generate_ref_sequence(int ref_len, int ref_seq_len );
char* generate_query_sequence(int n);
 ...
 * --------------------
 * Calculates element in scoring matrix and predecessor matrix
 *
 */
void calculate_element(int up, int diagonal, int left, int * score, int * pred) {
...
/*
 * Function: generate_query_sequence
 * --------------------
...
char* generate_ref_sequence(
  int ref_len,
  int ref_seq_len
) {
  char* ref = malloc(sizeof(char) * ((ref_len * ref_seq_len)+1));
  char seqChars[] = {'A','C','G','T'};
  for (int i; i<ref_len*ref_seq_len; i++) {
      ref[i] = seqChars[rand()%4];
  }
  ref[ref_len*ref_seq_len] = '\0';
  return ref;
}
